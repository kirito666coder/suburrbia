...
const VERTICAL_LINK_CLASSES = 'absolute top-0 h-full stroke-2 text-stone-300 transition-colors group-hover:text-stone-400'

const HORIZONTAL_LINK_CLASSES = ' -mx-8 stroke-2 text-stone-300 transition-colors group-hover:text-stone-400'
...
const SkateboardProduct = async({id}: Props) => {
  ...
  return (
    <div className='group relative mx-auto w-full max-w-72 px-8 pt-4 '>
      ...
      <VerticalLine className={clsx(VERTICAL_LINK_CLASSES, 'left-4')}/>
      <VerticalLine className={clsx(VERTICAL_LINK_CLASSES, 'right-4')}/>
      <HorizontalLine className={HORIZONTAL_LINK_CLASSES}/>
        ...
        <h3 className='my-2 text-center font-sans leading-tight text-lg md:text-xl font-extrabold'>
            {product.data.name}
        </h3>
